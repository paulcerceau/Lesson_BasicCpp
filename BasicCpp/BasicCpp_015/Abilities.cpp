...
int Abilities::GetAbilitiesDamage() const
{
	float strengthOutput = m_abilities.at(AbilityName::Strength) * 0.25f;
	float dexterityOutput = m_abilities.at(AbilityName::Dexterity) * 0.25f;

	return static_cast<int>(std::round(max(strengthOutput, dexterityOutput)));;
}
